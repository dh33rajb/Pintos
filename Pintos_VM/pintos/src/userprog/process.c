...
void process_exit(void) {
	...
	// Dheeraj: killing the suppl page table corresponding to the process
	hash_clear(&cur->suppl_page_table, hash_destructor_func);
...
// dheeraj: free frame and page before we go on to destroy a frame table entry
void hash_destructor_func(struct hash_elem *e, void *aux UNUSED) {
	struct page *pg = hash_entry(e, struct page, spt_hash_elem);
	if (pg->frame != NULL)
		frame_free(pg->frame);
	free(pg);
}
...
bool load(const char *cmd_line, void (**eip)(void), void **esp) {
	...
	// Dheeraj: Initializing suppl page table
	hash_init(&t->suppl_page_table, page_hash, page_less, NULL);
...
static bool load_segment(struct file *file, off_t ofs, uint8_t *upage,
	...
	while (read_bytes > 0 || zero_bytes > 0) {
		...
		struct page* pageentry = page_allocate(file, ofs, page_read_bytes,
				upage, writable);
		if (pageentry == NULL)
			return false;
...
// non-static method of the above one. Didn't want to change the storage class of the given one
...
// non-static method of load segment. **need to clean the static handling up (at later point in time)
// mem_map page table handling has been added here as-well
